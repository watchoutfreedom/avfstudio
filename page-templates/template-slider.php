...
<style>
    ...
    .post-body-content .wp-block-gallery {
        display: flex; flex-wrap: wrap; gap: 10px; margin: 1.5em 0;
    }
    .post-body-content .wp-block-gallery figure { flex: 1 1 150px; margin: 0; }
    .post-body-content blockquote {
        border-left: 3px solid #777; padding-left: 1.5em; margin: 1.5em 0; font-style: italic; color: #ddd;
    }
    .post-body-content blockquote p { margin-bottom: 0; }
...
<main class="concept-body" id="concept-body">
    ...
    <!-- REVISED: Robust PHP Query Logic -->
    <?php
    ...
    // --- Run Query 2 for remaining random posts if needed ---
    ...
    if ($remaining_needed > 0) {
        $random_args = array(
            ...
            'orderby'        => 'rand',
            'post__not_in'   => $exclude_ids, // Prevents duplicates
            'post_status'    => 'publish',
            'meta_query'     => array( array('key' => '_thumbnail_id') )
        );
    ...
    // --- Now, process the final, combined collection ---
    $initial_posts_data = [];
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // REVISED: window.onload now correctly handles hiding the loader
    window.onload = function() {
        randomizeInitialLayout(); // Lay out the cards first
        if (pageLoader) {
            setTimeout(() => { // A small delay feels smoother
