...
<style>
    /* All CSS from your working version is correct and unchanged, with additions for the new form. */
    ...
    .post-page.is-expanded {
        ...
        background-color: var(--expanded-bg, rgba(30, 30, 30, 0.97)); /* MODIFIED: Uses variable */
    }
    ...
    .card-content-view .brand-content { text-align: center; color: #ccc; }
    .card-content-view .brand-content a { color: #fff; /* ... */ }
    ...
    .propose-form-container input, .propose-form-container textarea {
        width: 100%; padding: 12px; margin-bottom: 15px; border: 1px solid #ccc;
        border-radius: 4px; font-size: 1rem; box-sizing: border-box; background-color: #f9f9f9;
    }
    .propose-form-container textarea { min-height: 150px; resize: vertical; }
    .propose-form-container .captcha-group { display: flex; align-items: center; margin-bottom: 20px; }
    .propose-form-container button[type="submit"] {
        width: 100%; padding: 15px; background-color: #333; color: #fff;
...
<!-- The old contact modal is no longer needed -->
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addCardFromButton() {
        if (availablePosts.length > 0) {
            ...
            // NEW: Define the full form HTML within the card's content
            const formHTML = `
                <div class="propose-form-container">
                    <h1>Propose Your Concept</h1>
                    <p>Tell us about your project. Please include a link or way of contact.</p>
                    <form id="propose-card-form">
                        <label for="propose-name">Name</label>
                        <input type="text" id="propose-name" name="name" required>
                        <label for="propose-email">Email</label>
                        <input type="email" id="propose-email" name="email" required>
                        <label for="propose-message">Your Concept</label>
                        ...
                        <button type="submit">Submit Concept</button>
    ...
    function expandCard(cardElement){
        ...
        // MODIFIED: Smart background switching
        ...
        // If it's the propose card, set up its form logic
        if (data.type === 'propose') {
            setupProposeForm();
        }
    }
    ...
    // NEW: Function to handle the self-contained form
    function setupProposeForm() {
        ...
        q1.textContent = n1; q2.textContent = n2;
        const answer = n1 + n2;
    ...
    function dragEnd() {
        ...
        if (!isDragging) {
            ...
            if (data) {
                // All card types now use the same expansion function
